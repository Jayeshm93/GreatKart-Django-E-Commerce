...
def add_cart(request, product_id):
    ...
    if curr_user.is_authenticated:
        ...
        is_cart_item_exists = CartItem.objects.filter(product=product, user=curr_user).exists()
        if is_cart_item_exists:
            cart_item = CartItem.objects.filter(product=product, user=curr_user)
            ...
            for item in cart_item:
                ...
                exist_ids.append(item.id)  # CartItem id
            ...
            if product_variation in exist_var_list:
                ...
                item_id = exist_ids[idx]  # Get specific existed variation CartItem id to increases the qty
                ...
                item = CartItem.objects.create(product=product, quantity=1, user=curr_user)
            ...
            cart_item = CartItem.objects.create(product=product, quantity=1, user=curr_user)
            # Add variation for that specific product
            if len(product_variation) > 0:
...
def remove_cart(request, product_id, cart_item_id):
    ...
    if curr_user.is_authenticated:
        ...
        try:
            cart_item = CartItem.objects.get(product=product, user=curr_user,
                                             id=cart_item_id)  # Get specific prod from this cart
...
def cart(request, total=0, quantity=0, cart_items=None):
    try:
        ...
        # If the user is authenticated
        if curr_user.is_authenticated:
            cart_items = CartItem.objects.filter(user=curr_user, is_active=True)
